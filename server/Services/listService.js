...
const create = async (model, user, callback) => {
	try {
		...
		ownerBoard.activity.unshift({
			user: user._id,
			name: user.name,
			action: `added ${newList.title} to this task`,
			color: user.color,
		});
...
const deleteById = async (listId, boardId, user, callback) => {
	try {
		...
		if (!validate) return callback({ errMessage: 'List or task informations are wrong' });

		// Validate whether the owner of the board is the user who sent the request.
		if (!user.boards.filter((board) => board === boardId))
			return callback({ errMessage: 'You cannot delete a list that does not hosted by your tasks' });
		...
		board.activity.unshift({
			user: user._id,
			name: user.name,
			action: `deleted ${result.title} from this task`,
			color: user.color,
		});
		board.save();
...
const updateCardOrder = async (boardId, sourceId, destinationId, destinationIndex, cardId, user, callback) => {
	try {
		...
		if (!validate || !validate2) return callback({ errMessage: 'List or task informations are wrong' });
