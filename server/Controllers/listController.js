...
const create = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ errMessage: 'You can not add a list to the task, you are not a member or owner!' });
...
const deleteById = async (req, res) => {
	...
	if (!(listId && boardId)) return res.status(400).send({ errMessage: 'List or task undefined' });
...
const updateCardOrder = async (req, res) => {
	...
	if (!validate) return res.status(403).send({ errMessage: 'You cannot edit the task that you hasnt' });
