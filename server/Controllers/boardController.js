...
const getById = async (req, res) => {
	...
	if (!validate)
		return res.status(400).send({ errMessage: 'You can not show the this task, you are not a member or owner!' });
...
const updateBoardTitle = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ errMessage: 'You can not change title of this task, you are not a member or owner!' });
...
const updateBoardDescription = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ errMessage: 'You can not change description of this task, you are not a member or owner!' });
	const { boardId } = req.params;
...
const updateBackground = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ errMessage: 'You can not change background of this task, you are not a member or owner!' });
	const { boardId } = req.params;
...
const addMember = async (req, res) => {
	...
	if (!validate)
		return res
			.status(400)
			.send({ errMessage: 'You can not add member to this task, you are not a member or owner!' });
	const { boardId } = req.params;
